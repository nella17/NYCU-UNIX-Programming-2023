...
When returning from execution, the debugger should disassemble 5 instruction starting from the current program counter. The address of the 5 instructions should be within the range of the text segment specified in the ELF file. We do not care about the format, but in each line, there should be 
...
2. [70%] We use `N` additional test cases to evaluate your implementation. You get $\frac{60}{N}$ points for each correct test case.
