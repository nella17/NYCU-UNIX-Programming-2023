...
int main(int argc, char const *argv[]) {
    ...
    for (long long n = 0; n < (1<<SIZE); n++) {
        if (ptrace(PTRACE_SETREGS, child, 0, &saved_regs) < 0) errquit("ptrace(POKEUSER)");
        ...
        for (int i = 0; i < 8; i++)
            buf[i % 8] = '0' + ((n >> i) & 1);
        if (ptrace(PTRACE_POKETEXT, child, magic, *(long*)buf) < 0) errquit("ptrace(POKETEXT)");
        memset(buf, 0, 8);
        for (int i = 8; i < SIZE; i++)
            buf[i % 8] = '0' + ((n >> i) & 1);
        if (ptrace(PTRACE_POKETEXT, child, magic+8, *(long*)buf) < 0) errquit("ptrace(POKETEXT)");
